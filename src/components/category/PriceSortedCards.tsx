...
export function MinimalQuoteCard({ item }: { item: FlatProductItem }) {
  ...
  return (
    <article
      className="flex flex-col justify-between rounded-xl border border-line bg-paper-2/70 p-4 shadow-xs transition-all hover:border-line-strong hover:bg-paper"
...
export function FlatPriceCard({ item }: { item: FlatProductItem }) {
  ...
  return (
    <TiltCard max={10} glare className="h-full rounded-card">
      <article
        className="depth-surface group relative flex h-full flex-col justify-between overflow-hidden rounded-card border border-line bg-paper p-5 md:p-6"
        aria-label={`${item.insurerZh} ${item.title}`}
      >
        <div>
          <div className="flex items-start justify-between gap-2">
            ...
            <div className="shrink-0 rounded-lg border border-line-strong/20 bg-paper-2/60 p-1 shadow-xs">
              <Card3DGem size={20} color={color} glow={false} />
            </div>
          </div>
