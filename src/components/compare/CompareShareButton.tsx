...
function ShareSession({ ids, className }: { ids: string[]; className?: string }) {
  ...
  return <>
    <button ref={button} type="button" onClick={copy} disabled={busy || ids.length === 0} aria-busy={busy} className={className ?? "btn-ghost min-h-11"} style={{ borderColor: "var(--line-strong)" }}>
      {copied ? <Check size={14} aria-hidden="true" /> : <Link2 size={14} aria-hidden="true" />}
      <span role="status" aria-live="polite">{busy ? "正在複製…" : copied ? "連結已複製" : "複製比較連結"}</span>
    </button>
    <Dialog.Root open={Boolean(manualUrl)} onOpenChange={open => { if (!open) setManualUrl(""); }}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-[101] w-[calc(100%-2rem)] max-w-lg -translate-x-1/2 -translate-y-1/2 rounded-card border bg-paper p-6 shadow-lift outline-none" data-lenis-prevent
          ...
          <label htmlFor="manual-compare-share" className="mt-5 block text-sm font-bold text-ink">比較分享網址</label>
          <input ref={field} id="manual-compare-share" value={manualUrl} readOnly onFocus={event => event.currentTarget.select()} className="mt-2 min-h-11 w-full rounded-lg border bg-paper-2 px-3 font-grotesk text-sm text-ink focus:ring-2 focus:ring-jade" />
