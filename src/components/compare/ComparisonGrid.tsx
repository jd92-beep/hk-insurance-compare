...
export default function ComparisonGrid({ products, generatedAt }: { products: Product[]; spare?: boolean; generatedAt?: string }) {
  ...
  const row = (label: string, render: (p: Product, i: number) => ReactNode) => <tr className="border-b border-line"><th scope="row" className="compare-label-col sticky left-0 z-10 min-w-0 border-r border-line bg-paper px-3 py-5 text-left align-top text-base font-semibold text-ink fold:px-4">{label}</th>{products.map((p,i) => <td key={p.id} className="min-w-0 px-3 py-5 align-top text-base leading-relaxed text-ink fold:px-5">{render(p,i)}</td>)}</tr>;
  return <div>
    ...
    <div className="overflow-x-auto rounded-xl border border-line" role="region" aria-label="保險資料比較表，可橫向捲動" tabIndex={0}>
      <table className="w-full min-w-[520px] table-fixed border-collapse fold:min-w-[640px] lg:min-w-[720px]">
        ...
        <thead><tr><th scope="col" className="compare-label-col border-b border-line bg-paper p-3 text-left text-base text-ink fold:p-4">核對項目</th>{products.map(p => <th key={p.id} scope="col" className="border-b border-line bg-paper p-3 text-left align-top text-base font-bold text-ink fold:p-4">{p.insurer_zh}<span className="mt-2 block text-sm font-normal leading-relaxed text-ink-soft">{p.product_name_zh || p.product_name}</span></th>)}</tr></thead>
        <tbody>
          {group('01 先睇級別同限制')}
          ...
          {group('02 再睇保障摘要')}
          {rows.map(benefit => <tr key={benefit.key} className="border-b border-line"><th scope="row" className="compare-label-col sticky left-0 z-10 min-w-0 border-r border-line bg-paper px-3 py-5 text-left align-top text-base font-semibold text-ink fold:px-4">{benefit.label}</th>{products.map((p,i) => <td key={p.id} className="px-3 py-5 align-top fold:px-5"><CoverageLimitCell limit={benefit.limits[i]} highlight={false} /></td>)}</tr>)}
