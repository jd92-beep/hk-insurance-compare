...
export default function MobileCompare({ products, onRemove }: { products: Product[]; onRemove?: (id: string) => void }) {
  ...
  return <div className="space-y-5">
    <section aria-label="已選產品" className="space-y-3">{products.map((p,i) => <article key={p.id} className="depth-surface rounded-xl border border-line bg-paper p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-base font-bold text-ink">{String.fromCharCode(65+i)} · {p.insurer_zh}</p><Link to={`/product/${p.id}`} className="mt-2 block text-base leading-relaxed text-jade underline">{p.product_name_zh || p.product_name}</Link></div>{onRemove && <button type="button" onClick={() => onRemove(p.id)} aria-label={`移除 ${p.product_name_zh || p.product_name}`} className="flex h-11 w-11 shrink-0 items-center justify-center rounded-lg border border-line text-ink hover:bg-paper-2"><X size={20} aria-hidden="true" /></button>}</div></article>)}</section>
    <p className="rounded-xl border border-amber/30 bg-amber/5 p-4 text-base leading-relaxed text-ink">先睇限制，再睇保費。同一問題下面逐份對照，唔使來回切換。未提供唔等於不保。</p>
