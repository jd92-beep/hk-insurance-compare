...
import { CoverageLimitCell, ExclusionThemesCell, ExclusionsCell, KeyTermsCell, PlanTiersCell, PremiumRangeCell } from '@/components/compare/cells';
...
export default function MobileCompare({ products, onRemove }: { products: Product[]; onRemove?: (id: string) => void }) {
  ...
  return <div className="space-y-5">
    ...
    <section className="rounded-xl border border-line bg-paper" aria-label="不保主題比較"><h2 className="border-b border-line bg-paper-2 px-4 py-4 text-lg font-bold text-ink">不保主題（字面檢索）</h2>{products.map(p => <div key={p.id} className="border-b border-line p-4 last:border-0"><h3 className="mb-3 text-base font-bold text-ink">{p.insurer_zh}</h3><ExclusionThemesCell product={p} /></div>)}</section>
