...
export default function SavedComparisons({selected,catalog,snapshotDate,onRestore}: {
  ...
  return <details className="group mb-8 min-w-0 rounded-2xl border border-jade/30 bg-paper shadow-card" data-saved-comparisons>
    ...
    <div className="border-t border-line p-4 md:p-6">
      <p className="max-w-3xl text-sm leading-relaxed text-ink-soft">保留產品選擇方便再比。重開用當前網站資料，唔係保存報價。只存產品識別同名稱；名稱請勿填個人或健康資料。</p>
      ...
      {!state.sets.length&&!state.issue&&<p className="py-4 text-sm text-ink-soft">未有收藏。只存喺呢部瀏覽器；清除瀏覽器資料會移除。</p>}
      <ul className="mt-4 grid grid-cols-1 gap-4 fold:grid-cols-2">
        {state.sets.map(set=>{
          ...
          return <li key={set.id} className="min-w-0 rounded-xl border border-line bg-paper-2/40 p-4" data-saved-set>
            ...
            {!!review.changed.length&&<p className="mt-3 rounded-lg bg-amber/10 p-3 text-sm">{review.changed.length} 款資料或來源指紋已變動；重開後請覆核，唔代表保單已改。</p>}
