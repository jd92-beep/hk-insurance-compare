...
export default function TiltCard({ children, className, max = 12, glare = true, perspective = 1200 }: {
  ...
  const lift = useSpring(enabled ? 10 : 0, { stiffness: 200, damping: 20 });
  ...
  return <div ref={stage} className={cn("tilt-stage perspective-card relative", className)} data-tilt-enabled={enabled ? "true" : "false"}
    ...
    onPointerLeave={() => { reset(); lift.set(enabled ? 10 : 0); }}
    onPointerCancel={() => { reset(); lift.set(enabled ? 10 : 0); }}
  ...
  >
    ...
    >
      <div className="relative h-full w-full" style={{ transform: "translateZ(28px)" }}>
        {children}
      </div>
      {glare && enabled && (
        <motion.div
