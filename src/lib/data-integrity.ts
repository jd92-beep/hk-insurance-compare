...
function optionalLifecycle(value: unknown, path: string): string | undefined {
  if (value === null || value === undefined) return undefined;
  if (typeof value !== "string") throw Error(`生命週期欄位 ${path} 必須是文字。`);
  return value;
}
...
export function parseInsuranceData(value: unknown): InsuranceData {
  ...
  const products: Product[]=value.products.map((p: unknown)=>{
    ...
    return {
      ...p, coverage, citations,
      premium_available:p.premium_available===true,
      record_status,
      last_verified_at: last_verified_at ?? null,
      source_document_version: source_document_version ?? null,
