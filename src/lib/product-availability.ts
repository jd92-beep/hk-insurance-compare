...
export function isReferenceOnlyProduct(product: Pick<Product, "id" | "record_status">): boolean {
  return product.id === "travel-aig" || product.record_status === "archived" || product.record_status === "discontinued";
}
export function purchaseUrl(product: Product): string | undefined {
  if (isReferenceOnlyProduct(product)) return undefined;
