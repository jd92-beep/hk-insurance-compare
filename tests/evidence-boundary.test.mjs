...
test('optional lifecycle fields are accepted when valid and stay unknown when absent', () => {
  ...
  const parsed = parseInsuranceData(raw);
  assert.equal(parsed.products[0].record_status, 'discontinued');
  assert.equal(parsed.products[0].last_verified_at, '2026-09-04');
  assert.equal(parsed.products[1].record_status, undefined);
  assert.equal(parsed.products[1].last_verified_at, null);
  const bad = snapshot(); bad.products[0].record_status = 99;
  assert.throws(() => parseInsuranceData(bad), /record_status/);
});
